...
export function StockChart({ type }: StockChartProps) {
  ...
  const drawLineChart = (
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      data[i] =
        i === 0
          ? 100
          : data[i - 1] *
            (1 + (Math.random() > 0.5 ? randomValue : -randomValue));
    }
    ...
    data.forEach((value, index) => {
      ...
      const y =
        height -
        ((value - minValue) / valueRange) * (height * 0.8) -
        height * 0.1;
  ...
  const drawCandlestickChart = (
    ...
    data.forEach((candle, index) => {
      ...
      const openY =
        height -
        ((candle.open - minValue) / valueRange) * (height * 0.8) -
        height * 0.1;
      const closeY =
        height -
        ((candle.close - minValue) / valueRange) * (height * 0.8) -
        height * 0.1;
      const highY =
        height -
        ((candle.high - minValue) / valueRange) * (height * 0.8) -
        height * 0.1;
      const lowY =
        height -
        ((candle.low - minValue) / valueRange) * (height * 0.8) -
        height * 0.1;
